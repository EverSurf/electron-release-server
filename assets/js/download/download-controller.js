angular
  ...
  .config([
    ...
    function ($routeProvider) {
      $routeProvider.when("/releases/:channel?/:flavor?", {
  ...
  .controller("DownloadController", [
    ...
    function (
      ...
      // Get selected channel from route or set to default (release)
      self.channel =
        $routeParams.channel ||
        (self.availableChannels && self.availableChannels[0]) ||
        "release";
      ...
      self.getLatestReleases = function () {
        self.setRouteParams(self.channel, self.flavor);
      ...
      self.download = function (asset, versionName, flavorName) {
        ...
        self.downloadUrl = `/download/flavor/${flavorName || flavor}/${
          versionName || version
        }/${platform}/${name}`;
      };
